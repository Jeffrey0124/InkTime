...
产品成功意味着：照片能被可靠扫描和 AI 分析，分析结果进入 SQLite；用户能在 WebUI 中按评分、日期、推送选片规则和随机发现浏览已分析照片；单张照片能生成与设备输出一致的 `800x480` PNG/BMP 预览；最终可写入 `/push/latest.bmp` 供设备拉取。
...
- 第一阶段是否完整实现鼠标拖动/滚轮缩放 Canvas 编辑器，或先保留参数控制。
...
3. 设备一致性优先于装饰：预览必须尽量还原最终 `800x480` 六色成品。
